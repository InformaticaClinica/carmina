...
Currently, Carmina is focused on processing the CARMEN corpus. For proper functioning, the following directory and file structure is expected within the `data/inputs/` folder:

```
data/inputs/
├── CARMEN1_mappings.tsv    # Mapping file for the CARMEN corpus
└── txt/
    ├── ann/                # Contains .ann files with annotations
    ├── identify/           # (Directory for identified entities, generated by the process)
    ├── masked/             # (Directory for masked text versions, generated by the process)
    └── raw/                # Contains raw text files corresponding to annotations
```

*   Place your `.ann` files in the `data/inputs/txt/ann/` folder.
*   The corresponding raw text files (`.txt`) must be in `data/inputs/txt/raw/`.
*   The `CARMEN1_mappings.tsv` file must be present in `data/inputs/`.
*   The `identify/` and `masked/` directories will be used by the application to save intermediate or final processing results, according to the configuration.
