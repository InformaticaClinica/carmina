...
class GoogleAIStudioProvider(BaseCloudProvider):
    ...
    def __init__(self, **kwargs):
        self.api_key = os.environ.get("GOOGLE_AI_STUDIO_API_KEY") or kwargs.get("api_key", None)
    ...
    def check_user_message(self, messages: list) -> bool:
        if len(messages) > 1:
            raise ValueError("Multiple user messages are not supported in Google AI Studio.")
        elif len(messages) == 0:
