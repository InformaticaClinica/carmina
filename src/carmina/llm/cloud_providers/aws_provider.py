...
class AWSProvider(BaseCloudProvider):
    ...
    def __init__(self, name: str = "aws", **kwargs):
        ...
        self.access_key_id = os.environ.get("AWS_ACCESS_KEY_ID") or kwargs.get(
            "access_key_id", None
        )
        self.secret_access_key = os.environ.get("AWS_SECRET_ACCESS_KEY") or kwargs.get(
            "secret_access_key", None
        )
        self.session_token = os.environ.get("AWS_SESSION_TOKEN") or kwargs.get(
            "session_token", None
        )
        ...
        self.base_delay = float(
            os.environ.get("AWS_BASE_DELAY", "2.0")
        )  # Increased from 1.0
        self.max_delay = float(
            os.environ.get("AWS_MAX_DELAY", "120.0")
        )  # Increased from 60.0
        self.request_delay = float(
            os.environ.get("AWS_REQUEST_DELAY", "1.0")
        )  # Increased from 0.1

        # Timeout configuration
        self.read_timeout = int(os.environ.get("AWS_READ_TIMEOUT", "120"))  # 2 minutes
        self.connect_timeout = int(
            os.environ.get("AWS_CONNECT_TIMEOUT", "60")
        )  # 1 minute
    ...
    def _create_bedrock_client(self):
        ...
        return self.session.client(service_name="bedrock-runtime", config=config)
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if isinstance(e, ClientError):
                    ...
                    if error_code in retryable_codes and attempt < self.max_retries:
                        logging.warning(
                            f"{error_code} on attempt {attempt + 1}/{self.max_retries + 1}. "
                            f"Will retry..."
                        )
                        continue
                    else:
                        # Re-raise non-retryable ClientErrors or if max retries exceeded
                        if attempt == self.max_retries:
                            logging.error(
                                f"Max retries exceeded for ClientError: {error_code}"
                            )
                        raise

                elif (
                    isinstance(e, (ReadTimeoutError, Urllib3ReadTimeoutError))
    ...
    def run_inference(
        self, model_id: str, messages: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "anthropic" in model_id:
                ...
                system = next(
                    (m["content"] for m in messages_list if m.get("role") == "system"),
                    "",
                )
                user = next(
                    (m["content"] for m in messages_list if m.get("role") == "user"), ""
                )
            ...
            logging.debug(
                f"Request body for {model_id}: {json.dumps(request_body, indent=2)}"
            )
            ...
            if isinstance(e, ClientError):
                ...
                if error_code == "ValidationException":
                    ...
                    logging.error(
                        f"Request body was: {json.dumps(request_body, indent=2)}"
                    )
            else:
        ...
        for i, messages in enumerate(batch_inputs):
            try:
                ...
                result = self.run_inference(
                    model_name, messages, inference_params=inference_params
                )
                ...
                if i < total_inputs - 1:  # Don't wait after the last request
                    logging.info(
                        f"Waiting {batch_delay:.1f} seconds before next request..."
                    )
                ...
                if i < total_inputs - 1:
                    ...
                    logging.info(
                        f"Error occurred, waiting {error_delay:.1f} seconds before next request..."
                    )
                ...
                if i < total_inputs - 1:
                    error_delay = batch_delay * 2
                    logging.info(
                        f"Unexpected error occurred, waiting {error_delay:.1f} seconds before next request..."
                    )
                    time.sleep(error_delay)
